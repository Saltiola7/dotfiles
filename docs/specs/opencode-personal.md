...
**`qwen/qwen3.6-35b-a3b` stock prompt template crashes on tool-result-terminated conversations.** Symptom: `Error rendering prompt with jinja template: "No user query found in messages."`, returned on the first tool-calling turn. Root cause is in the model's own Jinja chat template (confirmed by reproducing the error hitting LM Studio `:1234` directly, bypassing HeadroomProxy entirely — so neither headroom nor compression is implicated). The template's "last user query" detector loop only clears its `multi_step_tool` guard when it sees a message with `role == "user"` whose content is *not* wrapped in `<tool_response>...</tool_response>`. OpenAI-style tool results arrive as `role == "tool"` (or as a `<tool_response>`-wrapped user), so a conversation that ends on a tool result with no qualifying user turn leaves `multi_step_tool` true and hits the template's hard `raise_exception('No user query found in messages.')`. Compression can also produce this shape if it drops/merges the leading user turn, which is why it appeared to fire "immediately after the first grep."

Fix: override the model's prompt template in LM Studio (My Models → `qwen/qwen3.6-35b-a3b` → Prompt Template), then unload/reload the model so the override takes effect. The override is stored at `~/.cache/lm-studio/.internal/user-concrete-model-default-config/qwen/qwen3.6-35b-a3b.json` (runtime cache — not chezmoi-managed; model-version-specific). The patched template is checked in at `docs/specs/assets/qwen3.6-35b-a3b.prompt-template.jinja`. Two changes vs. stock:
1. Detector loop: added an `elif ns.multi_step_tool and message.role == "tool"` branch that clears the guard and sets `last_query_index`, so OpenAI-style tool results count as a query anchor.
2. Hardened the post-loop guard: replaced the hard `raise_exception('No user query found in messages.')` with a safe fallback (`set ns.multi_step_tool = false` + `last_query_index = messages|length - 1`) so no message shape can crash rendering.

Reload after editing the template: `~/.cache/lm-studio/bin/lms unload qwen/qwen3.6-35b-a3b && ~/.cache/lm-studio/bin/lms load qwen/qwen3.6-35b-a3b`. Verify with the four-case smoke test (artificial tool-terminated, `<tool_response>`-wrapped-user-only, realistic system→user→assistant(tool_call)→tool, and plain user) — all must return `OK`.

**HEADROOM_MIN_TOKENS / STABLE_AFTER_TURN are tuning, not the jinja fix.** The `ai.headroom.proxy.lmstudio.plist` sets `HEADROOM_MIN_TOKENS=20000` and `HEADROOM_COMPRESSION_STABLE_AFTER_TURN=6` to avoid compressing a small local model's context prematurely. These were initially (mis)attributed to the jinja error; they are kept as sensible local-model hygiene but the actual root cause was the model template above.
