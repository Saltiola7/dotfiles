...
def test_marimo_uses_pinned_catppuccin_mocha():
    ...
    assert (
        'custom_css = ["{{ .chezmoi.homeDir }}/.config/marimo/themes/'
        'catppuccin-latte-mocha.css"]' in config
    )

    theme = text(
        "private_dot_config/marimo/themes/catppuccin-latte-mocha.css"
    )
    assert "Generated by catppuccin/whiskers" in theme
    assert "--background: light-dark(#eff1f5, #1e1e2e);" in theme
    assert "--primary: light-dark(#8839ef, #cba6f7);" in theme
